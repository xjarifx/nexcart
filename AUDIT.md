...
### 6. Prisma client singleton has no connection pooling config
**Severity: Critical — ops**
...
### 7. No customer-facing order cancellation
...
### 11. Seed script stores plaintext passwords
**Severity: High bug**
...
### 12. CORS is hardcoded to a single origin
...
### 13. No input sanitization on UUID path parameters
...
### 15. Category deletion has no child/product guard
**Severity: High — data integrity**
...
### 16. Config doesn't validate ImageKit variables
...
### 18. Category slug not checked for uniqueness on update
**Severity: Medium bug**
...
### 25. No review-purchase verification
...
### 26. Swagger docs exposed in production
**Severity: Medium — security**
...
### 29. No Prisma error handling for known constraint violations
**Severity: Medium — developer experience**
...
### 33. CI pipeline exists but has no type-checking step
...
### 36. No health check for database connectivity
**Severity: Low — ops**
...
### 37. `package.json` has `name: "server"` with no description
...
### 38. Dockerfile runs migrations on every container start
...
### 39. Swagger spec uses hardcoded `./src/modules/**/*.route.ts` glob
...
### 43. `pino-http` logger initialized before `logger` import
